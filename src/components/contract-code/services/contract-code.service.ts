...
export class ContractCodeService {
  ...
  async registerContractCode(
    ctx: RequestContext,
    request: RegisterContractCodeParamsDto,
  ): Promise<any> {
    ...
    if (contractCodeDb) {
      ...
      await lastValueFrom(
        this.httpService.post(
          `${this.indexerUrl}${INDEXER_API.REGISTER_CODE_ID}`,
          properties,
        ),
      ).then((rs) => rs.data);
  ...
  async updateContractCode(
    ctx: RequestContext,
    codeId: number,
    request: UpdateContractCodeParamsDto,
  ): Promise<any> {
    this.logger.log(
      ctx,
      `${this.updateContractCode.name} was called! ${codeId} ${request.type} ${this.indexerChainId}`,
    );
    try {
      //check exist code id in db
      const contractCode = await this.smartContractCodeRepository.findOne({
      ...
      if (contractCode) {
        ...
        if (result !== CONTRACT_CODE_RESULT.CORRECT) {
          ...
          this.logger.log(
            ctx,
            `Call Indexer with parameter indexer: ${properties}`,
          );
          await lastValueFrom(
            this.httpService.post(
              `${this.indexerUrl}${INDEXER_API.REGISTER_CODE_ID}`,
      ...
      this.logger.error(
        ctx,
        `Class ${ContractCodeService.name} call updateContractCode method error: ${err.stack}`,
      );
      throw err;
    }
  }
