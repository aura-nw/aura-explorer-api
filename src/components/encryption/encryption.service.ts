...
import { ENCRYPTION_CONFIG_OPTIONS } from './encryption-options.type';
...
export class EncryptionService {
    ...
    constructor(
        ...
        @Inject(ENCRYPTION_CONFIG_OPTIONS) private options: EncryptionOptions
    ) {
    ...
    public async addCipherKey() {
        ...
        const { CiphertextBlob } = await this.kms.generateDataKey({ KeyId, KeySpec }).promise();
    ...
    public async printKey() {
        ...
        const KeySpec = 'AES_256';
        const { CiphertextBlob, Plaintext } = await this.kms.generateDataKey({ KeyId, KeySpec }).promise();
    ...
    public async getKey(): Promise<PlaintextType> {
        const cipherKey = await this.cipherKeyRepository.findOne();
