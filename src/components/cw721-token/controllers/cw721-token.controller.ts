...
export class Cw721TokenController {
  ...
  async getCw721Tokens(
    ...
    @Body() request: Cw721TokenParamsDto,
  ): Promise<any> {
    this.logger.log(ctx, `${this.getCw721Tokens.name} was called!`);
    const { tokens, count } = await this.cw721TokenService.getCw721Tokens(
      ctx,
      request,
    );
  ...
  async getNftByContractAddressAndTokenId(
    ...
    const nft = await this.cw721TokenService.getNftByContractAddressAndTokenId(
      ctx,
      contractAddress,
      tokenId,
    );
  ...
  async getNftsByOwner(
    @ReqContext() ctx: RequestContext,
    @Body() request: NftByOwnerParamsDto,
  ): Promise<any> {
