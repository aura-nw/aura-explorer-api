...
export class SoulboundTokenController {
    ...
    async getSoulboundContracts(
        ...
        this.logger.log(ctx, `============== ${this.getSoulboundContracts.name} was called! ==============`);
        const { contracts, count } = await this.soulboundTokenService.getContracts(ctx, query);

        return { data: contracts, meta: count };
    }

    @Get('tokens')
    @ApiOperation({ summary: 'Get list tokens by minter address and contract address' })
    @ApiResponse({ status: HttpStatus.OK, schema:{} })
    async getTokens(
        @ReqContext() ctx: RequestContext,
        @Query() req: TokenParasDto) {
        this.logger.log(ctx, `============== ${this.getTokens.name} was called with paras: ${JSON.stringify(req)}! ==============`);
        const {data, count} =  await this.soulboundTokenService.getTokens(ctx, req);
        return { data, meta: count };
    }

    @Post()
    @ApiOperation({ summary: 'Create token of Soulbound contract which user can clain or mint token' })
    @ApiResponse({ status: HttpStatus.OK, schema:{} })
    async create(@ReqContext() ctx: RequestContext, @Body() req: CreateSoulboundTokenParamsDto) {
