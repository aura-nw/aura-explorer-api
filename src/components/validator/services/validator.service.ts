...
export class ValidatorService {
  ...
  async getValidators(
    ...
    // get all validator
    const validatorsRes: Validator[] =
      await this.validatorRepository.getValidators();
  ...
  async getDelegations(
    ...
    let accountData = await this.serviceUtil.getDataAPI(
    ...
    if (accountData.data === null) {
      accountData = await this.serviceUtil.getDataAPI(
        `${this.indexerUrl}${util.format(
          INDEXER_API.ACCOUNT_DELEGATIONS,
          delegatorAddress,
          this.indexerChainId,
