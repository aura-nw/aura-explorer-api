    ...
    async getContracts(request: ContractParamsDto) {
        ...
        if (request?.keyword) {
            ...
            if (Number(keyword) && Number(keyword) > 0) {
                ...
                params.push(keyword);
            } else if (keyword.startsWith(AURA_INFO.CONTRACT_ADDRESS) && keyword.length === LENGTH.CONTRACT_ADDRESS) {
                sql += ` WHERE (sc.contract_address = ? OR sc.creator_address = ?)`;
                params.push(keyword);
                params.push(keyword);
            } else if (keyword.startsWith(AURA_INFO.CONTRACT_ADDRESS) && keyword.length === LENGTH.ACCOUNT_ADDRESS) {
